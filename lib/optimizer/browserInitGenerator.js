...
exports.generate = function(groupMap, params) {
  /*
  ...
  */
  ...
  try {
    return requirejsConfig.collect(params.targetDir).zip(Future.call(fs.readFile, path.join(params.targetDir, templateFile), 'utf8')).map(function(requireConf, tmplString) {
      return tmplString.replace('COMPUTED_REQUIREJS_CONFIG', JSON.stringify(requireConf, null, 2)).replace('GROUP_MAP', JSON.stringify(groupMap, null, 2));
    }).failAloud();
  } catch (_error) {
    e = _error;
