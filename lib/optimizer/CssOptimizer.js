...
var CleanCss, CorrelationGroupDetector, CssOptimizer, Future, GroupRepo, HeuristicGroupDetector, _, cleanCss, fs, path, relativeReplaceRe, sha1;
...
CssOptimizer = (function() {
  ...
  * and so on
  ...
  CssOptimizer.prototype.run = function() {
    ...
    return Future.call(fs.readFile, cssStatFile).then((function(_this) {
  ...
  CssOptimizer.prototype._generateOptimizationMap = function(stat) {
    var corrDetector, group, groupId, groupRepo, groups, heuristicDetector, iterations, l, len, page, ref, resultMap;
    iterations = 1;
    ...
    for (page in stat) {
      groups = stat[page];
      for (l = 0, len = groups.length; l < len; l++) {
        groupId = groups[l];
  ...
  CssOptimizer.prototype._generateOptimizedFiles = function(groupMap) {
    ...
    return this._mergeGroups(groupMap).then((function(_this) {
      return function(mergedGroupMap) {
        var css, cssToGroup, fileName, groupId, l, len, urls;
        cssToGroup = {};
        for (groupId in mergedGroupMap) {
          urls = mergedGroupMap[groupId];
          for (l = 0, len = urls.length; l < len; l++) {
            css = urls[l];
            cssToGroup[css] = groupId;
          }
        }
        fileName = _this.params.targetDir + "/conf/css-to-group-generated.js";
        return Future.call(fs.writeFile, fileName, "define(function(){ return " + (JSON.stringify(cssToGroup, null, 2)) + "; });").then(function() {
  ...
  CssOptimizer.prototype._mergeGroups = function(groupMap) {
     ...
     */
    var cssFiles, groupId, mergePromises, resultMap;
    resultMap = {};
    mergePromises = (function() {
      ...
      for (groupId in groupMap) {
        ...
        results.push(this._mergeGroup(this._reorderGroupFiles(cssFiles)).spread(function(fileName, existingFiles) {
  ...
  CssOptimizer.prototype._mergeGroup = function(cssFiles) {
     ...
     */
    var contentArr, existingFiles, file, futures, j, removePromises, savePromise;
    existingFiles = [];
    contentArr = [];
    removePromises = [];
    futures = (function() {
      var l, len, results;
      results = [];
      for (j = l = 0, len = cssFiles.length; l < len; j = ++l) {
        file = cssFiles[j];
        results.push((function(_this) {
          return function(file, j) {
            var filePath;
            filePath = _this.params.targetDir + "/public" + file;
            return Future.call(fs.readFile, filePath, 'utf8').then(function(origCss) {
              var css, fileBaseUrl;
              if (_this.params.removeSources) {
                removePromises.push(Future.call(fs.unlink, filePath));
              }
    ...
    savePromise = Future.all([Future.all(futures), this.zDirFuture]).then((function(_this) {
      return function() {
        ...
        return Future.call(fs.writeFile, _this._zDir + "/" + fileName + ".css", mergedContent).then(function() {
  ...
  CssOptimizer.prototype._calculateGlobalOrder = function(stat) {
     ...
     */
    var file, files, i, k, keys, l, len, len1, m, p, prepends, preps, preps1, prevLength, ready, result, s;
    prepends = {};
    for (p in stat) {
      files = stat[p];
      for (i = l = 0, len = files.length; l < len; i = ++l) {
        file = files[i];
    ...
    while ((keys = Object.keys(prepends)) && keys.length > 0 && keys.length < prevLength) {
      ...
      for (m = 0, len1 = ready.length; m < len1; m++) {
        k = ready[m];
        delete prepends[k];
      }
      for (file in prepends) {
