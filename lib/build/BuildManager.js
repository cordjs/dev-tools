...
var BuildManager, BuildWorker, Future, os, _;
...
BuildWorker = require('./BuildWorker').BuildWorker;
...
BuildManager = (function() {
  ...
  */
  ...
  BuildManager.MAX_WORKERS = Math.max(os.cpus().length - 1, 2);
    ...
    */
    ...
    if (emptyWorker) {
      ...
      if (this.workers.length < this.MAX_WORKERS) {
        newWorker = new BuildWorker(this);
  ...
  BuildManager.stopWorker = function(worker) {
    ...
    return console.log("BuildWorker " + worker.id + " stopped. Total tasks count: " + worker.totalTasksCount);
  };
