...
ProjectBuilder = (function(_super) {
  ...
  ProjectBuilder.prototype.build = function() {
    ...
    this._previousSessionPromise = completePromise["catch"](function() {
      return true;
    });
    return completePromise.always(function(err) {
      var diff, verb;
      diff = process.hrtime(start);
      verb = err ? 'failed' : 'complete';
      return console.log("Build " + verb + " in " + ((diff[0] * 1e9 + diff[1]) / 1e6) + " ms");
    }).then((function(_this) {
      return function() {
        buildManager.stop();
