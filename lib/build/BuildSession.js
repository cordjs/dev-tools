...
BuildSession = (function() {
  ...
  BuildSession.prototype.add = function(file) {
    ...
    if (file === BuildSession.APP_CONF_FILE + '.coffee') {
      ...
      return this._appConfPromise["finally"]((function(_this) {
        return function() {
          return _this._handleFile(file, fileInfo.detectBundle(file))["catch"](function() {
  ...
  BuildSession.prototype._handleFile = function(file, bundle) {
    ...
    if (info.isStylus) {
      return this._pathUtilsPromise.then((function(_this) {
        return function() {
      ...
      if (info.isWidget) {
        return this._corePromise.then((function(_this) {
          return function() {
            return _this._createTask(file, info);
          };
        })(this)).link(this._widgetClassesPromise);
      } else if (info.isWidgetTemplate) {
        return this._widgetClassesPromise.then((function(_this) {
          return function() {
            return _this._createTask(file, info);
          };
        })(this));
  ...
  BuildSession.prototype._createTask = function(file, info) {
    return this._sourceModified(file, info).then((function(_this) {
  ...
  BuildSession.prototype._sourceModified = function(file, info) {
    ...
    return Future.all([Future.call(fs.stat, srcPath), Future.call(fs.stat, dstPath)]).spread(function(srcStat, dstStat) {
