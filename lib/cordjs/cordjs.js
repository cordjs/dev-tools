...
var CommandGenerator, Generator2, bundleGenerator, colors, createDir, exec, fs, path, projectGenerator, sendCommand, spawn, sys, util, utils, _ref,
  __slice = [].slice;
...
CommandGenerator = {
  widget: function(commander) {
    return commander.command('widgetCreate [widgetName]').description('create widget in current directory').option("-B, --behaviour", "create widget with BehaviourClass").option("-C, --css", "create with css").action(function(widgetName, options) {
...
sendCommand = function(command, callback) {
  return exec(command, function(error, stdout, stderr) {
    if (stderr) {
      utils.printWarn(stderr);
    }
    if (stdout) {
      utils.printLine(stdout);
    }
