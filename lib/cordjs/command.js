...
mainCommand = function() {
  ...
  Cordjs.sendCommand("mkdir -p " + outputDir, function(error) {
  ...
  return initCompileWidgets = function(callback) {
    var baseUrl, configPaths, dirname, extname, source, widgetsPaths, _i, _len;
    ...
    requirejs(["cord!config"], function(config) {
      return config.PUBLIC_PREFIX = baseUrl;
    });
...
compileWidget = function(callback) {
  ...
  return requirejs(["cord!config", "cord-w!" + widgetName, "cord!widgetCompiler"], function(WidgetClass, widgetCompiler, config) {
    var widget;
