...
var CoffeeScript, Cordjs, EmptyArguments, Stylus, StylusLib, aFiles, addWidgetWaitCompiler, baseDirFull, basePath, commander, compileWidget, config, copyFile, createDir, debugMode, exec, exists, existsSync, fs, getWidgetPath, hidden, iErrServerStart, isDiffByTimeSource, isDiffSource, isServerRestart, mainCommand, nib, outputDir, outputPath, packageInfo, path, pathToCore, pathToNodeInit, port, printLine, printWarn, publicDir, removeDirSync, removeSource, removeUnusedFiles, requirejs, restartServer, serverChild, sources, spawn, startServer, stopServer, syncFile, syncFiles, testCommandDir, timeEnd, timeStart, timerErrServer, unwatchDir, util, wait, walk, watchDir, watchFile, watchModeEnable, widgetsWaitCompiler, _endTimer, _getStat, _ref, _startTimer;
...
commander.option('-a, --autorestart', 'autorestart server').option('-b, --build', 'build project').option('-c, --clean', 'clean target').option('-d, --dev', 'development mode - copy all files to the outputDir').option('-o, --output [dir]', 'output directory [' + outputDir + ']', outputDir).option('-f, --config [name]', 'configuration file name [' + config + ']', config).option('-p, --port [port]', 'server listening port [' + port + ']', port).option('-s, --server', 'start server').option('-w, --watch', 'watch scripts for changes and rerun commands').option('-g, --debug', 'flag for node debugging').version(packageInfo.version, '-v, --version');
...
commander.command('core [env]').description('     create'.grey + ' - create Core\n' + '     update'.grey + ' - pulling from github\n' + '     remove'.grey + ' - remove Core').action(function(env) {
  if (!testCommandDir()) {
    return false;
  }
  switch (env) {
    case 'update':
      if (!fs.existsSync('public/bundles/cord/core')) {
        return Cordjs.utils.timeLogError('Can\'t find cord/core!');
      }
      Cordjs.utils.timeLog('Update core...');
      return Cordjs.sendCommand("git --git-dir=public/bundles/cord/core/.git pull", function() {
        return Cordjs.utils.timeLog('Update core complete!');
      });
    case 'create':
      if (fs.existsSync('public/bundles/cord/core')) {
        return Cordjs.utils.timeLogError('Can\'t create cord/core, because core is exist!');
      }
      return Cordjs.sendCommand("mkdir -p public/bundles/cord/core", function(error) {
        Cordjs.utils.timeLog('Cloning core...');
        return Cordjs.sendCommand("git clone https://github.com/cordjs/core.git public/bundles/cord/core");
      });
    case 'remove':
      return Cordjs.sendCommand("rm -R public/bundles/cord/core", function(error) {
        return Cordjs.utils.timeLog('Core remove complete!');
      });
    default:
      return Cordjs.utils.timeLog('Nothing todo');
  }
});
...
mainCommand = function() {
  ...
  return initCompileWidgets = function(callback) {
    ...
    return requirejs(["pathUtils"], function(pathUtils) {
...
addWidgetWaitCompiler = function(source) {
  var canonicalPath, structure, structureStat;
  if (~source.indexOf('/widgets/') && source.substr(-5) === '.html') {
    canonicalPath = getWidgetPath(source);
    structure = "" + source + ".structure.json";
    structureStat = _getStat(structure);
    if (widgetsWaitCompiler.indexOf(canonicalPath) === -1 && (structureStat == null) && isDiffSource(source, outputPath(structure, basePath))) {
      return widgetsWaitCompiler.push(canonicalPath);
    }
  }
};
...
compileWidget = function(callback) {
  ...
  return requirejs(["cord-w!" + widgetName, "cord!widgetCompiler"], function(WidgetClass, widgetCompiler) {
    ...
    return widget.compileTemplate(function(err, output) {
      var source, sourceOutput, structure, structureOutput;
      if (err) {
        throw err;
      }
      ...
      return fs.writeFile(structureOutput, widgetCompiler.getStructureCode(false), function(err) {
        var _this = this;
        fs.stat(source, function(err, stat) {
          fs.utimes(sourceOutput, stat.atime, stat.mtime);
          return fs.utimes(structureOutput, stat.atime, stat.mtime);
        });
        return compileWidget(callback);
      });
    });
...
removeUnusedFiles = function(callback) {
  var sourcePath, targetPublicPath, walker;
  targetPublicPath = path.join(outputDir, publicDir);
  walker = walk.walkSync(targetPublicPath, {
    followLinks: false
  });
  sourcePath = function(root, name) {
    var source;
    source = path.join(root, name);
    return source = source.replace(targetPublicPath, publicDir);
  };
  walker.on('directory', function(root, stat, next) {
    var source;
    source = sourcePath(root, stat.name);
    exists(source, function(itExists) {});
    return next();
  });
  walker.on('file', function(root, stat, next) {
    var currentSource, extname, filename, source;
    ...
    if (extname === '.js' && !~source.indexOf('html.js')) {
      (function(filename, source, currentSource) {
        return exists("" + filename + ".coffee", function(itExists) {
          if (!itExists) {
            return exists(source, function(itExists) {
              if (!itExists) {
                return fs.unlink(currentSource, function(err) {
                  if (err && err.code !== 'ENOENT') {
                    throw err;
                  }
                  return Cordjs.utils.timeLog("removed unused file " + currentSource);
                });
              }
            });
          }
        });
      })(filename, source, currentSource);
    }
    return next();
  });
  return walker.on('end', function() {
...
copyFile = function(source, base, callback, symbolicLink) {
  ...
  copyHelper = function() {
    return fs.stat(source, function(err, stat) {
      ...
      if (extname === '.coffee') {
        ...
        re = /^@import ['"](.*\/\/.+)['"]$/gm;
        return fs.readFile(source, 'utf8', function(err, str) {
          var pathUtils, replaced;
          if (err) {
            throw err;
          }
          pathUtils = require("" + (path.join(baseDirFull, publicDir, pathToCore)) + "requirejs/pathUtils");
          replaced = str.replace(re, function(match, p1) {
            return "@import \"" + (pathUtils.convertCssPath(p1, source)) + "\"";
          });
          return Stylus(replaced).set('filename', source).use(StylusLib).set('compress', true).render(function(err, css) {
            if (err) {
  ...
  return createDir(fileDir, function() {
    return copyHelper();
  });
};
...
isDiffSource = function(baseSource, outputSource, baseStat, outputStat) {
  if (baseStat == null) {
    baseStat = _getStat(baseSource);
  }
  if (outputStat == null) {
    outputStat = _getStat(outputSource);
  }
  if (outputStat != null) {
    if (!outputStat.isDirectory() && !path.extname(baseSource) === '.coffee' && !path.extname(baseSource) === '.styl' && outputStat.size !== baseStat.size) {
      return true;
    }
...
isDiffByTimeSource = function(baseSource, outputSource, baseStat, outputStat) {
  if (baseStat == null) {
    baseStat = _getStat(baseSource);
  }
  if (outputStat == null) {
...
watchFile = function(source, base, symbolicLink) {
  var e, prevStats, rewatch, sync, syncTimeout, watchErr, watcher;
  prevStats = null;
  syncTimeout = null;
  watchErr = function(e) {
    if (e.code === 'ENOENT') {
...
removeDirSync = function(source) {
  var e, file, filename, stat, _i, _len, _ref1;
  try {
