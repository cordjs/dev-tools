...
addWidgetWaitCompiler = function(source) {
  var canonicalPath;
  console.log("addWidgetWaitCompiler -> " + source);
  if (source.indexOf('/widgets/') !== -1 && source.substr(-5) === '.html') {
    ...
    if (widgetsWaitCompiler.indexOf(canonicalPath) === -1 && isDiffSource(path.dirname(source), outputPath(path.dirname(source), basePath))) {
      console.log("addWidetWaitCompiler OK");
...
compileWidget = function(callback) {
  var widgetName,
    _this = this;
  console.log("Compile widget start!");
  widgetName = widgetsWaitCompiler.pop();
  if (!(widgetName != null)) {
    return typeof callback === "function" ? callback() : void 0;
  }
  console.log("compileWidget " + widgetName + " ");
    ...
    return fs.stat(source, function(err, stat) {
      ...
      if (extname === '.coffee') {
        ...
        return fs.readFile(source, 'utf8', function(err, str) {
          ...
          return Stylus("@import 'nib' \n\n" + replaced).set('filename', source).define('url', Stylus.url()).use(nib()).render(function(err, css) {
            if (err) {
