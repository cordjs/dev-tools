...
var EmptyArguments, OptionsList, baseDirFull, copyFile, countFiles, exec, exists, fs, hidden, options, optparse, outputDir, outputPath, parseOptions, path, pathToNodeInit, printLine, printWarn, publicDir, removeDirSync, removeSource, server, sources, spawn, syncFile, syncFiles, timeLog, unwatchDir, usage, util, wait, walk, watchDir, watchFile, _ref;
...
OptionsList = [['-b', '--build', 'build project'], ['-d', '--dev', 'development mode - copy all files to the outputDir'], ['-h', '--help', 'display this help message'], ['-o', '--output [DIR]', 'output directory'], ['-s', '--server', 'start server'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  return completeSync = function() {
    return exec("coffee -bc -o " + outputDir + " server.coffee", function() {
      ...
      if (options.server) {
        ...
        return server.init(path.join(baseDirFull, outputDir, publicDir));
      }
    });
  };
};
  ...
  watchErr = function(e) {
    if (e.code === 'ENOENT') {
      ...
      try {
        ...
        return timeLog("Remove file '" + source + "'");
      }
    } else {
      throw e;
    }
